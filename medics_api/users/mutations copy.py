...
class LoginUserMutation(graphene.Mutation):
    """
    Authenticates a user and returns JWT tokens along with user details.
    """
    class Arguments:
        input = LoginUserInput(required=True, description="Input for user login.")

    token = graphene.String(description="The JWT access token.")
    print(f"Attempting to get refresh token for user: {user.username}")
    refresh_tok = get_refresh_token(user)
    print(f"Refresh token obtained: {refresh_tok is not None}")
    refresh_token = graphene.String(description="The JWT refresh token.")
    user = graphene.Field(UserType, description="The authenticated user object.")
    success = graphene.Boolean(description="Indicates if the login was successful.")
    errors = graphene.List(graphene.String, description="List of errors if the login failed.")

    @classmethod
    def mutate(cls, root, info, input):
        try:
            # Determine if login is by email or username
            if '@' in input.username_or_email:
                user = UserModel.objects.get(email=input.username_or_email)
            else:
                user = UserModel.objects.get(username=input.username_or_email)
            ...
            return cls(
                ...
                user=None,
                success=False,
                errors=["Invalid credentials"] # More general error for security
            )
            ...
            return cls(
                ...
                user=None,
                success=False,
                errors=[f"An unexpected error occurred: {str(e)}"] # Provide more context for debugging
            )
